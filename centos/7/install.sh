...
yum install php56w php56w-opcache php56w-common
yum install php56w-mcrypt php56w-gd php56w-pdo php56w-pear php56w-gettext php56w-xml php56w-mysql php56w-intl php56w-mbstring php56w-bcmath php56w-ldap  php56w-pecl-memcached 
yum -y install openssl mysql tre libzip sysstat mysql-connector-python && \
yum -y install tcp_wrappers memcached catdoc libpst   poppler-utils unrtf tnef gcc make which cronie && \
...
curl -L https://bitbucket.org/jsuto/piler/downloads/piler-1.3.0.tar.gz  | tar --strip-components=1 -xzC /usr/src/piler && \
